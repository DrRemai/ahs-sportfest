...
class RoundRobin implements FormatInterface
{
    public function generate(int $tournamentId, array $teamIds, array $config): void
    {
        ...
        if ($groups > 1) {
            ...
            for ($gi = 0; $gi < $groups; $gi++) {
                ...
                foreach ($schedule as $roundIdx => $pairs) {
                    ...
                    foreach ($pairs as [$home, $away]) {
                        $insertStmt->execute([$tournamentId, $stageId, 'none', $side, $round, $matchNum, $matchNum, $home, $away]);
                        $matchNum++;
                    }
                }
            }
            ...
            foreach ($schedule as $roundIdx => $pairs) {
                ...
                $matchNum = 1;
                foreach ($pairs as [$home, $away]) {
                    $insertStmt->execute([$tournamentId, $stageId, 'none', null, $round, $matchNum, $matchNum, $home, $away]);
                    $matchNum++;
                }
            }
        }
    ...
    /**
     * Return standings split by group.
     * Uses group_label column to discover groups and filter team membership.
    ...
    public function standingsByGroup(int $tournamentId, ?int $stageId = null): array
    {
        ...
        $gStmt = db()->prepare(
            "SELECT DISTINCT group_label FROM matches
        ...
        foreach ($sides as $side) {
            ...
            $tStmt = db()->prepare(
                "SELECT home_team_id FROM matches
                 WHERE tournament_id = ? $stageClause AND group_label = ? AND home_team_id IS NOT NULL
                 UNION
                 SELECT away_team_id FROM matches
                 WHERE tournament_id = ? $stageClause AND group_label = ? AND away_team_id IS NOT NULL"
            );
            $tStmt->execute($p);
